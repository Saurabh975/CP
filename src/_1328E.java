...
public class _1328E {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        while (t-- > 0) {
            k = ni();
            int inp[] = new int[k];
            for (int i = 0; i < k; inp[i] = ni(), i++) ;
            //pl(Arrays.toString(inp));
            int u = inp[0];
            for (int x : inp) if (depth[u] <= depth[x]) u = x;

            for (int x = 0; x < k; x++) {
                if (inp[x] == u) continue;
                if (par[inp[x]] != -1) inp[x] = par[inp[x]];
            }
            //pl(Arrays.toString(inp));
            boolean flag = true;
            for (int x : inp) flag &= isAncestor(x, u);
            if (flag) pl("YES");
            else pl("NO");
