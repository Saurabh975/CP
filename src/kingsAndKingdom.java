...
public class kingsAndKingdom {
    ...
    static char rec(long ar[], long pos, int l) {
        //pl(pos+" "+l);
        if (l == 1) return ch1[(int) pos - 1];
        if (l == 2) return ch2[(int) pos - 1];
        if (pos <= ar[l - 2]) return rec(ar, pos, l - 2);
        if(pos > ar[l - 1] + ar[l - 2]) return rec(ar, pos - ar[l - 2] - ar[l - 1], l - 1);
        return rec(ar, pos - ar[l - 2], l - 1);
    }
