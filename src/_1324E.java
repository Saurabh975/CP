...
public class _1324E {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        l = ni();
        ar = new int[n];
        for (int i = 0; i < 2005; i++) Arrays.fill(dp[i], -1);
        for(int i=0;i < n;i++)ar[i] = ni();
        pl(Khojo_BHAI(0, 0));
    }

    static int Khojo_BHAI(int index, int last){

        if(index == n){
            if(last>=k && last <= l)return 1;
            return 0;
        }

        if(dp[index][last] != -1)return dp[index][last];

        dp[index][last] = max(Khojo_BHAI(index + 1, (last+ar[index]) % m),
                Khojo_BHAI(index + 1, (last + ar[index] - 1 + m) % m));

        if(index > 0 && last<= l && last >= k)dp[index][last]++;
