...
public class helpMilo {
    static long m[][] = new long[4][4];
    static long a[][] = new long [4][4];
    static long idn[][] = new long[4][4];
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        t=ni();
        idn[0][0]=idn[1][1]=idn[2][2]=idn[3][3]=1;
        while(t-->0) {
            long n = nl();
            m = new long[4][4];
            a = new long[4][4];
    ...
    static long[][] matexp(long m[][],long pow) {
//        if (pow == 0) return idn;
//        long res[][] = new long[4][4];
//        for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) res[i][j] = m[i][j];
//        while (pow > 0) {
//
//            res = matmul(res, res);
//
...
//        return res;
        ...
        if (pow == 0) return idn;
        if (pow == 1) return m;
        if ((pow & 1) == 1) return matmul(m, matexp(matmul(m, m), pow >> 1));
        return matexp(matmul(m, m), pow >> 1);
    }
