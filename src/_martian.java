...
public class _martian {
    ...
    static int laura_lassan(int i, int j) {
        ...
        dp[i][j][0] = laura_lassan(i - 1, j) + ar[i][j];
        dp[i][j][1] = laura_lassan(i, j - 1) + br[i][j];

        return dp[i][j][2] = max(dp[i][j][0], dp[i][j][1]);
    }
