...
public class _2B {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        ans = new int[n + 1][n + 1];
        dp[n][n] = ar[n][n];
        //pl(samjhe_ki_nahi(1,1));
        pl(laddu_gino(samjhe_ki_nahi(1, 1)));
        int i = 1, j = 1;
        while (i + j < n << 1) {
            if (ans[i][j] == 1) {
                pws("D");
...
//        }
    ...
    static long samjhe_ki_nahi(int i, int j) {
        ...
        int x = -1;
        long cur_min = 12345678;

        long temp_min1 = samjhe_ki_nahi(i + 1, j);
        if (temp_min1 != -1) {
            temp_min1 *= ar[i][j];
            int c = laddu_gino(temp_min1);
        ...
        long temp_min2 = samjhe_ki_nahi(i, j + 1);
        if (temp_min2 != -1) {
            temp_min2 *= ar[i][j];
            int c = laddu_gino(temp_min2);
            if (c <= cur_min) {
                cur_min = c;
                x = 2;
            }
        ...
        ans[i][j] = x;
        return dp[i][j] = (x == 1 ? temp_min1 : temp_min2);
    }

    static int laddu_gino(long x) {
