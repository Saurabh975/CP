...
public class _4D {
    ...
    static int dsu[] = new int[5001];
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {

        n = ni();
        k = ni();
        l = ni();
        ar.add(new data(0, 0, -1));
        for (int i = 1; i <= n; i++) {
            ar.add(new data(ni(), ni(), i));
        }
        int start = -1;
        long cur_max = -1234567;
        ar.sort((data a, data b) -> (int)a.a - (int)b.a);
        //pl(ar);
        for (int i = 1; i <= n; i++) {
            if (ar.get(i).a > k && ar.get(i).b > l) {
                long temp = laura_lassan(i, ar.get(i).a, ar.get(i).b);
                if (temp > cur_max) {
                    start = i;
                    cur_max = temp;
                }
    ...
    static long laura_lassan(int i, long w, long h) {
        long ans = 0;

        if (dp[i] != 0) return dp[i];

        for(int j = i+1 ;j <= n; j++){
            if(ar.get(j).a > w && ar.get(j).b > h){
                long temp = laura_lassan(j, ar.get(j).a, ar.get(j).b);
