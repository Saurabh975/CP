...
public class _sqrbr {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        while (t-- > 0) {
            ...
            k = ni();
            ar = new int[2 * n + 2];
            for (int i = 0; i < (n << 1) + 1; i++) Arrays.fill(dp[i], -1);
            for (int i = 1; i <= k; i++) ar[ni()] = 1;
    ...
    static int Laura_Lassan(int ind, int sum) {
        if (ind == 2 * n) {
            if (sum == 0) {
                return 1;
            }
        ...
        dp[ind][sum]++;
        dp[ind][sum] += Laura_Lassan(ind + 1, sum + 1);
        if (ar[ind + 1] != 1 && sum != 0) dp[ind][sum] += Laura_Lassan(ind + 1, sum - 1);
