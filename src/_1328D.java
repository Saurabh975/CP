...
public class _1328D {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < n; i++) set.add(ar[i] = ni());
            if (set.size() <= 2 || n % 2 == 0) {
                if (set.size() == 1) {
                    pl(1);
                    ...
                    if(n%2==0) {
                        pl(2);
                        for (int i = 0; i < n; i++) p(i % 2 + 1);
                        pl();
                        continue here;
                    }
                    pl(2);
                    for (int i = 0; i < n; i++) p(ar[i] == set.first() ? 1 : 2);
                    pl();
                }
            } else {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (ar[i] == ar[(i + 1) % n]) {
                        pl(2);
                        for (int j = 1, pos = i + 1; pos < n;) {
                            ar[pos++] = (j ^=1) + 1;
                        }
                        for (int j = 1, pos = i; pos >= 0;) {
                            ar[pos--] = (j ^=1) + 1;
                        }
                        for (int k = 0; k < n; k++) p(ar[k]);
                        pl();
                        continue here;
                    }
                }
