...
static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
    ...
    while (t-- > 0) {
        ...
        k = ni();
        int ar[] = new int[n + 1];
        int kr[] = new int[(k << 1) + 2];
        HashMap<Integer, Integer> map = new HashMap<>();
        for (int i = 1; i <= n; i++) ar[i] = ni();

        for (int i = 1; i <= n >> 1; i++) {
            int j = n - i + 1;
            int max = max(ar[i], ar[j]) + k + 1;
            int min = min(ar[i], ar[j]) + 1;

            kr[min]++;
            kr[max]--;

            map.put(max + min - k - 2, map.getOrDefault(max + min - k - 2, 0) + 1);
        }
        ...
        for (int i = 1; i <= k << 1; i++) ans = min(ans, n - kr[i] - map.getOrDefault(i, 0));
        pl(ans);
    }
}
