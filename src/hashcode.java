...
public class hashcode {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        Comparator<library> comp = (library a, library b) -> b.n*a.sin-a.n*b.sin;
        PriorityQueue<library> chacha = new PriorityQueue<>(comp);

        for (int i = 0; i < l; i++) {
            library ob = new library(i, ni(), ni(), ni());
            for (int j = 0; j < ob.n; j++) {
                int x = ni();
                ob.pq.add(new data(x, score[x]));
            }
            chacha.add(ob);
        }
...
//        library ot = chacha.poll();
//        chacha.poll();
//        chacha.add(ot);

        while (!chacha.isEmpty() && day > chacha.peek().sin) {
            c++;
            library ob = chacha.poll();
            day -= ob.sin;
            long books = min(ob.n, day * ob.w);
            int tempbook = 0;
            //ans.append(ob.id + " " + books + "\n");
            //pl(ob.id+" "+books);
            StringBuilder temp = new StringBuilder("");
            while (books-- > 0 && ob.pq.size() > 0) {
                if (set.contains(ob.pq.peek().a)) {
                    set.remove(ob.pq.peek().a);
                    temp.append(ob.pq.poll().a + " ");
                    tempbook++;
                } else ob.pq.poll();
            }
            if (tempbook > 0) {
                ans.append(ob.id + " " + tempbook + "\n" + temp + "\n");
            } else c--;
        }
        pl(c + "\n" + ans);
    }
    ...
    static class library {
        PriorityQueue<data> pq = new PriorityQueue<>(c);
        int id , n, sin, w;

        library(int w, int x, int y, int z) {
            n = x;
            sin = y;
            this.w = z;
            id = w;
    ...
    static Comparator<data> c = (data a, data b) -> b.b - a.b;
    ...
    public static void main(String[] args) {      //threading has been used to increase the stack size.
        try {
            ...
            input = new AwesomeInput(new FileInputStream("/home/saurabh/Desktop/c.txt"));
