...
public class xterm {
    ...
    static void MainSolution() {
        n=ni();
        char ar[][]=new char[n][];
        for(int i=0;i<n;i++)ar[i]=ns().toCharArray();
        int q = ni();
        while(q-->0) {
            int l = ni() - 1;
            int r = ni() - 1;
            ...
            for (int i = l; i <= r; i++) {
                int j;
                for (j = 0; j < min(ch.length, ar[i].length); j++) {
                    ans = min(ans, ar[i].length + ch.length - j - j);
                    if(ch[j]!=ar[i][j])break;
                }
                if(j==ch.length)ans = min(ans, ar[i].length + ch.length - j - j);
            }
            pl(ans);
        }
    }
