...
public class _1204B {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int flag = 0;
            for (int k = 0; k < sb.length() && flag == 0; k++) {
                if (sb.charAt(k) != temp.charAt(k)) flag = 1;
            }
            if (flag == 1)
            rev.add(sb.toString());
        }
        ...
        for (String i : ar) {
            if (set.contains(i)) {
                if (rev.contains(i)) {
                    sb.append(i);
                    set.remove(i);
                    set.remove(new StringBuilder(i).reverse().toString());
                } else {
                    String temp = new StringBuilder(i).reverse().toString();
                    int flag = 0;
                    for (int k = 0; k < i.length() && flag == 0; k++) {
                        if (i.charAt(k) != temp.charAt(k)) flag = 1;
                    }
                    if (flag == 0) ind = t;
                }
            }
        ...
        if(ind == -1){
            ...
            sb = new StringBuilder(sb.toString() + ar.get(ind) + sb.reverse().toString());
        }
