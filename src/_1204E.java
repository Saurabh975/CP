...
public class _1204E {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        while (t-- > 0) {
            ...
            int len_ab = length(a, b, lca(a, b));

            int min_len_ab_xy = length(a, x, lca(a, x)) + length(b, y, lca(b, y));
            min_len_ab_xy = min(min_len_ab_xy, length(a, y, lca(a, y)) + length(b, x, lca(b, x))) + 1;
            int ans = Integer.MAX_VALUE;

            if (len_ab % 2 == k % 2) ans = len_ab;

            if (min_len_ab_xy % 2 == k % 2) ans = min(ans, min_len_ab_xy);
        ...
        //pl(Arrays.toString(level));
    }

    static int length(int a, int b, int lca) {
    ...
    static void dfs(int i, int p, int lev) {
        parent[i][0] = p;
        tin[i] = ++timer;
        level[i] = lev;
        for (int j = 1; j <= l; j++)
