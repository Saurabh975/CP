...
public class _submerge {
      static LinkedList<Integer> adj[];
      static boolean vis[];
      static int tin[], low_tin[];
      ...
      static void dfs(int u, int p) {
            ...
            for (int v : adj[u]) {
                  ...
                  if (vis[v]) {
                        low_tin[u] = min(low_tin[u], tin[v]);
                  } else {
                        children++;
                        dfs(v, u);
                        low_tin[u] = min(low_tin[u], low_tin[v]);

                        if (low_tin[v] >= tin[u] && p != -1) set.add(u);

                  }
            }

            if (p == -1 && children > 1) set.add(u);
